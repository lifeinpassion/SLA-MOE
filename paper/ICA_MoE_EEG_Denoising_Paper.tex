...
\title{Self-Learning ICA-Initialized Mixture of Experts for Adaptive EEG Artifact Removal}
...
\item \textbf{Comprehensive Evaluation}: We conduct extensive experiments demonstrating that the ICA-based initialization is essential for performance---the same MoE architecture without our initialization strategy shows substantially degraded results, validating the importance of our self-learning approach.
\end{enumerate}
...
Combining ICA with neural networks has been explored in various contexts. ICA-based feature extraction has been used as preprocessing for neural network classifiers \cite{makeig2004mining}. Some works have used ICA to initialize convolutional filters, improving convergence and performance on image classification tasks \cite{le2011ica}. However, to our knowledge, no prior work has used ICA component characteristics to guide the specialization of expert networks in an MoE framework for EEG denoising.
...
A key innovation of our approach is the self-learning pre-training strategy that leverages ICA characteristics to create expert-specific training targets. This enables each expert to specialize in handling different aspects of the signal without explicit artifact labels.

For each expert $e \in \{0, 1, 2, 3\}$, we define target signals based on ICA component characteristics:

\textbf{Expert 0 (Low-Frequency Specialist)}: Targets components with high low-frequency power ($\rho_{\text{low}} > 0.5$):
\begin{equation}
\mathbf{t}_0 = 0.7 \cdot \mathbf{s} + 0.3 \cdot \text{Reconstruct}(\mathbf{x}, \mathcal{I}_{\text{low}})
\end{equation}
where $\mathcal{I}_{\text{low}} = \{k : \rho_{\text{low},k} > 0.5\}$.

\textbf{Expert 1 (High-Frequency Specialist)}: Targets components with high-frequency power ($\rho_{\text{high}} > 0.5$):
\begin{equation}
\mathbf{t}_1 = 0.75 \cdot \mathbf{s} + 0.25 \cdot \text{Reconstruct}(\mathbf{x}, \mathcal{I}_{\text{high}})
\end{equation}

\textbf{Expert 2 (Artifact Specialist)}: Targets non-Gaussian components indicative of artifacts:
\begin{equation}
\mathbf{t}_2 = \mathbf{s} - 0.2 \cdot (\hat{\mathbf{a}} - \bar{\mathbf{a}})
\end{equation}
where $\hat{\mathbf{a}} = \text{Reconstruct}(\mathbf{x}, \mathcal{I}_{\text{ng}})$ with $\mathcal{I}_{\text{ng}} = \{k : |\kappa_k| > 1 \text{ or } |\gamma_k| > 1\}$.

\textbf{Expert 3 (Ensemble Specialist)}: Weighted combination of all components:
\begin{equation}
\mathbf{t}_3 = 0.8 \cdot \mathbf{s} + 0.2 \cdot \sum_k w_k \cdot \text{Reconstruct}(\mathbf{x}, \{k\})
\end{equation}
where $w_k = (1 + |\kappa_k| + |\gamma_k|)^{-1}$ weights cleaner components higher.

During pre-training, experts are optimized independently with an \textbf{independence loss} to encourage diverse specialization:
\begin{equation}
\mathcal{L}_{\text{pre}}^{(e)} = \|\mathbf{y}^{(e)} - \mathbf{t}_e\|_2^2 + \lambda_{\text{ind}} \sum_{e' < e} |\text{corr}(\mathbf{y}^{(e)}, \mathbf{y}^{(e')})|
\end{equation}
where $\lambda_{\text{ind}} = 0.1$ encourages decorrelated expert outputs.
...
We evaluate our method on a real EEG dataset consisting of:
\begin{itemize}
\item \textbf{Clean EEG}: 512 epochs $\times$ 512 time samples from artifact-free segments
\item \textbf{EOG Reference}: Corresponding electrooculogram recordings capturing eye movement artifacts
\item \textbf{EMG Reference}: Electromyogram recordings capturing muscle activity artifacts
\end{itemize}
...
We employ six metrics to comprehensively evaluate denoising performance:
...
We compare against both traditional and deep learning methods:

\textbf{Traditional Methods}:
\begin{itemize}
\item Least Mean Squares (LMS) adaptive filter \cite{widrow1975adaptive}
\item Recursive Least Squares (RLS) filter \cite{haykin2014adaptive}
\item Kalman filter \cite{morbidi2008application}
\end{itemize}

\textbf{Deep Learning Methods}:
\begin{itemize}
\item EEGDnoiseNet \cite{zhang2021eegdenoisenet}: 10-layer dilated CNN with residual connections
\item RNN-EEG: Bidirectional LSTM with attention mechanism
\item ResNet-EEG: 1D residual network with encoder-decoder structure
\item SimpleCNN: 3-layer convolutional baseline
\end{itemize}

\subsection{Implementation Details}

Our ICA-MoE model uses:
\begin{itemize}
\item Number of experts: 4
\item Hidden size: 128
\item ICA components: 4
\item Top-$k$ selection: $k=2$
\item Pre-training epochs: 5
\item Main training epochs: 10
\item Batch size: 32
\item Optimizer: Adam (lr=$10^{-3}$)
\end{itemize}

All experiments are repeated with 5 random seeds (40-44) for statistical reliability. Results report mean $\pm$ standard deviation.
...
Our ICA-MoE achieves remarkable performance improvements across all metrics:
\begin{itemize}
\item \textbf{RMSE}: 1.59, which is 52.1\% lower than the best baseline (SimpleCNN: 3.32)
\item \textbf{Correlation}: 0.99998, indicating near-perfect signal reconstruction
\item \textbf{SNR}: 43.19 dB, representing 6.43 dB improvement over SimpleCNN
\item \textbf{SNR Improvement}: +6.18 dB, while all deep learning baselines show negative values
\item \textbf{Spectral Distortion}: 0.005, preserving spectral characteristics better than alternatives
\end{itemize}
...
Key observations:
\begin{itemize}
\item \textbf{EOG Removal}: Achieves the lowest RMSE (1.45) and highest single-artifact SNR improvement (3.93 dB), consistent with EOG artifacts' more structured nature (low-frequency, high-amplitude).
\item \textbf{EMG Removal}: Shows higher variance (std = 1.11 for SNR) due to EMG's broader spectral content and higher complexity.
\item \textbf{Combined Removal}: Despite handling more complex contamination, achieves the highest absolute SNR improvement (6.18 dB), demonstrating the MoE architecture's ability to jointly address multiple artifact types.
\end{itemize}

The adaptive gating mechanism enables the model to route signals to appropriate experts based on the predominant artifact characteristics, explaining the robust performance across scenarios.
...
\textbf{How ICA Enables Self-Learning Specialization}: Our key insight is that ICA decomposition provides natural signal characterizations that can guide expert specialization \textit{without requiring manual artifact labels}. By analyzing component properties:
\begin{itemize}
\item \textbf{Spectral features} (low/high frequency power ratios) distinguish slow neural oscillations from fast muscle artifacts
\item \textbf{Statistical features} (skewness, kurtosis) identify non-Gaussian artifact sources from more Gaussian neural activity
\end{itemize}

These characteristics allow us to automatically construct expert-specific training targets that guide each expert toward complementary behaviors during pre-training.
...
The MoE paradigm has gained significant attention in the context of large language models (LLMs), where sparse MoE enables scaling to trillions of parameters \cite{fedus2022switch, jiang2024mixtral}. Our work demonstrates that similar principles apply to EEG signal processing:

\begin{itemize}
\item \textbf{Conditional computation}: Activating only relevant experts reduces computational cost while maintaining model capacity
\item \textbf{Expert specialization}: Domain-specific pre-training (using ICA in our case) guides meaningful expert roles
\item \textbf{Load balancing}: Auxiliary losses ensure stable training and prevent expert collapse
\end{itemize}

These insights suggest that MoE architectures have broader applicability in biomedical signal processing beyond the specific EEG denoising task.
...
\begin{itemize}
\item Extension to multi-channel EEG with spatial expert specialization
\item Online adaptation for real-time BCI applications
\item Transfer learning across datasets and artifact types
\item Integration with downstream tasks (e.g., seizure detection, sleep staging)
\end{itemize}
...
\bibitem{chi2022representation}
Z. Chi, L. Dong, S. Huang, D. Dai, S. Ma, B. Patra, S. Singhal, P. Bajaj, X. Song, X.-L. Mao, H. Huang, and F. Wei, ``On the representation collapse of sparse mixture of experts,'' in \textit{Proc. Advances in Neural Information Processing Systems (NeurIPS)}, vol. 35, pp. 34600--34613, 2022.
